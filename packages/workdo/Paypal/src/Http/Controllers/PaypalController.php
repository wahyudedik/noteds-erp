...
class PaypalController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                    ...
                    $beautybooking->payment_option = 'Paypal';
    ...
    public function GetPaypalWaterParkPaymentStatus(Request $request, $slug)
    {
        try {
            ...
            try {
                ...
                $e = __('Transaction has been failed');
                return redirect()->route('water-park-frontend.booking', [$slug])->with('error', $e);
            }
        } catch (\Exception $e) {
    ...
    // eBook
    public function bookPayWithPaypal(Request $request, $slug)
    {
        try{
            $reader = \Auth::guard('reader')->user();
            $this->paymentConfig($reader->created_by, $reader->workspace);
            $currency             = $this->currancy;
            $supported_currencies = ['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'JPY', 'CNY', 'CHF', 'HKD', 'SGD', 'INR', 'BRL', 'BRL', 'MXN', 'ZAR'];
            ...
            $company_settings = getCompanyAllSetting($reader->created_by, $reader->workspace);
            $response         = $provider->createOrder([
                "intent"              => "CAPTURE",
                "application_context" => [
                    "return_url" => route('ebook.paypal.status', $slug),
                    "cancel_url" => route('ebook.paypal.status', $slug),
                ],
                "purchase_units" => [
                    0 => [
                        "amount" => [
    ...
    public function getPaypalBookPaymentStatus(Request $request, $slug)
    {
        try
        {
            $reader    = Auth::guard('reader')->user();
            ...
            return redirect()->route('ebook.frontend', $slug)->with('error',  __('Payment failed or cancelled.'));
        }catch(Exception $e)
        {
            return redirect()->back()->with('error', $e->getMessage());
        }
    ...
    // YogaClasses
    public function YogaCoursesPayWithPaypal(Request $request, $slug)
    {
        ...
        if ($products) {
            try {
                if ($total_price <= 0) {
                    $assignCourse = YogaUtility::DirectAssignCourse($workspace, $products, 'Paypal');
                    if ($assignCourse['is_success']) {
                        return redirect()->route(
                ...
                $response = $provider->createOrder([
                    "intent" => "CAPTURE",
                    "application_context" => [
                        "return_url" => route('yoga.courses.payment.status', ['slug' => $slug]),
                        "cancel_url" => route('yoga.courses.payment.status', ['slug' => $slug]),
                    ],
    ...
    public function getYogaCoursesPaymentStatus(Request $request, $slug)
    {
        $workspace = WorkSpace::where('slug', $slug)->first();
        ...
        if ($products) {
            ...
            $payment_id = Session::get('paypal_payment_id');
            try {
                $order = new YogaCourseOrder();
                $latestOrder = YogaCourseOrder::orderBy('created_at', 'DESC')->first();
                if (!empty($latestOrder)) {
                    $order->order_nr = '#' . str_pad($latestOrder->id + 1, 4, "100", STR_PAD_LEFT);
                } else {
                    $order->order_nr = '#' . str_pad(1, 4, "100", STR_PAD_LEFT);
                ...
                if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                    ...
                    event(new PaypalPaymentStatus($product, $type, $course_order));
    ...
    public function InvestDepositPayWithPaypal(Request $request, $slug)
    {
        ...
        if ($price > 0) {
            $this->paymentConfig($investor->created_by, $investor->workspace);
            $currency               = $this->currancy;
            $supported_currencies   = ['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'JPY', 'CNY', 'CHF', 'HKD', 'SGD', 'INR', 'BRL', 'BRL', 'MXN', 'ZAR'];
            ...
            try {
                ...
                $response = $provider->createOrder([
                    "intent" => "CAPTURE",
                    "application_context" => [
                        "return_url" => route('invest.deposit.paypal.status', ['slug' => $slug] + $data),
                        "cancel_url" => route('invest.deposit.paypal.status', ['slug' => $slug] + $data),
                    ],
                    "purchase_units" => [
                        0 => [
                            "amount" => [
    ...
    public function getPaypalInvestDepositPaymentStatus(Request $request, $slug)
    {
        try {
            $workspace = WorkSpace::where('slug', $slug)->first();
            $this->paymentConfig($workspace->created_by, $workspace->id);
            ...
            try {
                if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                    ...
                    $existingInvestment = InvestorDeposit::where('investor_id', $investor->id)
                                        ->where('plan_id', $investor_plan->id)
                                        ->first();
                        ...
                        if ($existingInvestment) {
                            ...
                            $investor_deposit->plan_duration = $request->plan_duration;;
                            $investor_deposit->invested_amount =$request->amount;
                            $investor_deposit->amount =$request->amount;
                            $investor_deposit->annual_return =$request->annual_return;
                            $investor_deposit->expected_return = $request->expected_return;
                            $investor_deposit->payment_type = 'Paypal';
                            $investor_deposit->status = '1';
                            $investor_deposit->purchase_date = $request->purchase_date;
                            $investor_deposit->expiry_date =$request->expiry_date;
                    ...
                    $investor_transaction->detail      = 'Deposit Via - Paypal';
                    ...
                    event(new PaypalPaymentStatus($investor_deposit, $type, $investor_transaction));
