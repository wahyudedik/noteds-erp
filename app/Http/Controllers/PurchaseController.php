...
class PurchaseController extends Controller
{
    ...
    public function purchaseLink($purchaseId)
    {
        ...
        if (!empty($purchase)) {
            ...
            if (module_is_active('CustomField')) {
                $purchase->customField = \Workdo\CustomField\Entities\CustomField::getData($purchase,'pos', 'purchase');
                $customFields = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace($purchase->created_by))->where('module', '=', 'pos')->where('sub_module', 'purchase')->get();
            } else {
                $customFields = null;
            }
            return view('purchases.customer_bill', compact('purchase', 'vendor', 'iteams', 'purchasePayment', 'user', 'company_id', 'workspace_id','customFields'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
