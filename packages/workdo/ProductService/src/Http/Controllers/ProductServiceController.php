...
class ProductServiceController extends Controller
{
    ...
    public function show($id)
    {
        if (\Auth::user()->isAbleTo('product&service show')) {
            ...
            if (module_is_active('CustomField')) {
                $productService->customField = \Workdo\CustomField\Entities\CustomField::getData($productService, 'ProductService', 'Product & Service');
                $customFields = \Workdo\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'ProductService')->where('sub_module', 'Product & Service')->get();
            } else {
                $customFields = null;
            }

            return view('product-service::view', compact('productService', 'vendors', 'purchases', 'total_sold', 'total_cost', 'total_products_use', 'productslogtime', 'products', 'customFields'));
        } else {
            return redirect()->back()->with('error', __("Permission Denied"));
        }
    }
