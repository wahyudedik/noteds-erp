...
use Workdo\YogaClasses\Entities\PurchasedYogaCourse;
use Workdo\YogaClasses\Entities\YogaCourseOrder;
use Workdo\YogaClasses\Entities\YogaInstructor;
use Workdo\YogaClasses\Entities\YogaMember;
use Workdo\YogaClasses\Entities\YogaUtility;
use Workdo\BeautySpaManagement\Entities\BeautyServiceOffer;
use Workdo\InvestmentSystem\Entities\Investor;
use Workdo\InvestmentSystem\Entities\InvestorDeposit;
use Workdo\InvestmentSystem\Entities\InvestorTransaction;
use Workdo\InvestmentSystem\Entities\InvestPlan;
...
class StripeController extends Controller
{
    ...
    public function BeautySpaPayWithStripe(Request $request, $slug, $lang = '')
    {
        ...
        if ($service) {
            try {
                ...
                ) ? number_format($finalPrice, 2, '.', '') : number_format($finalPrice, 2, '.', '') * 100;
                $return_url_parameters = function ($return_type) {
                    return '&return_type=' . $return_type;
                };
                /* Initiate Stripe */
   ...
   public function getBeautySpaPaymentStatus(Request $request, $slug)
    {
        ...
        try {
            if ($request->return_type == 'success' && !empty(session()->get('beauty_spa_variable'))) {
                ...
                $service = BeautyService::find($service_id);
                $price = $service->price ?? 0;
                ...
                $finalPrice = BeautyBooking::total_amount($data['person'], $price);
                ...
                $beautybooking->payment_option  = 'Stripe';
                $beautybooking->reference       = $data['reference'];
                $beautybooking->notes           = $data['notes'];
                ...
                return redirect()->route('beauty.spa.booking.confirm', [
                    'slug' => $slug,
                    'id'   => Crypt::encrypt($beautybooking->id),
                ])->with('success', __('Your Service has been booked successfully.'));
        ...
        try {
            if ($request->return_type == 'success' && !empty(session()->get('bookings_variable'))) {
                ...
                if (!empty($bookingscustomer)) {
                    ...
                    $bookingscustomer->last_name   = isset($data['last_name']) ? $data['last_name'] : $bookingscustomer->last_name;
                    $bookingscustomer->description = isset($data['description']) ? $data['description'] : $bookingscustomer->description;
                    ...
                    $bookingscustomer->last_name   = isset($data['last_name']) ? $data['last_name'] : '';
                    $bookingscustomer->description = isset($data['description']) ? $data['description'] : '';
                ...
                return redirect()->route('bookings.frontend.appointment', $slug)->with('payment_success', $msg);;
            } else {
                $msg = __('Transaction has been failed');
                return redirect()->back()->with('msg', $msg);
            }
    ...
    public function getWaterParkPaymentStatus(Request $request, $slug)
    {
        ...
        try {
            if ($request->return_type == 'success' && !empty(session()->get('water_park_variable'))) {
                ...
                $msg = __('Payment has been success.');
                return redirect()->route('water-park-frontend.booking', [$slug])->with('success', $msg);
            } else {
                $msg = __('Transaction has been failed');
                return redirect()->route('water-park-frontend.booking', [$slug])->with('error', $msg);
            }
        } catch (\Exception $exception) {
            $msg = __('Transaction has been failed');
            return redirect()->route('water-park-frontend.booking', [$slug])->with('error', $msg);
   ...
   //ebook
   public function bookPayWithStripe(Request $request, $slug)
   {
       $reader = \Workdo\Ebook\Entities\EbookReader::findOrFail($request->reader_id);
       self::payment_setting($reader->created_by, $reader->workspace);

       $workspace = WorkSpace::where('slug', $slug)->firstOrFail();
       $admin_currency = $this->currancy;
       $supported_currencies = ['EUR', 'GBP', 'USD', 'CAD', 'AUD', 'JPY', 'INR', 'CNY', 'SGD', 'HKD', 'BRL'];

       if (!in_array($admin_currency, $supported_currencies)) {
           return redirect()->back()->with('error', __('Currency is not supported.'));
       }

       // Fetch cart items from the DB
       $cartItems = \Workdo\Ebook\Entities\EbookReaderCart::where('reader_id', $reader->id)->pluck('book_id');
       if ($cartItems->isEmpty()) {
           return redirect()->back()->with('error', __('Your cart is empty.'));
       }

       $carts = \Workdo\Ebook\Entities\EbookBook::whereIn('id', $cartItems)->get();
       if ($carts->isEmpty()) {
           return redirect()->back()->with('error', __('No valid books found in the cart.'));
       }
       ...
       try {
           $stripe_formatted_price = in_array(
               company_setting('defult_currancy', $reader->created_by, $reader->workspace),
               [
                   'MGA',
                   'BIF',
                   'CLP',
           ...
           /* Initiate Stripe */
           \Stripe\Stripe::setApiKey(company_setting('stripe_secret', $reader->created_by, $reader->workspace));
           $code = '';
           $orderId = '#' . time();
           $comapany_stripe_data = \Stripe\Checkout\Session::create(
               [
                   ...
                   'metadata' => [
                       'user_id' => isset($reader->first_name) ? $reader->first_name : 0,
                       'package_id' => $slug,
                       'code' => $code,
                   ],
                   'success_url' => route(
   ...
   public function getStripeBookPaymentStatus(Request $request, $slug)
   {
       if ($request->return_type == 'success') {
           $reader = auth('reader')->user();

           if (!$reader) {
               return redirect()->route('ebook.frontend', $slug)
               ->with('error', __('Reader not authenticated.'));
           }

           $workspace = WorkSpace::where('slug', $slug)->first();

           $cartItems = \Workdo\Ebook\Entities\EbookReaderCart::where('reader_id', $reader->id)->get();
           ...
           foreach ($cartItems as $item) {
               $book = \Workdo\Ebook\Entities\EbookBook::find($item->book_id);
               $author = \Workdo\Ebook\Entities\EbookAuthor::find($book->author_id);

               if ($book && !\Workdo\Ebook\Entities\EbookPurchasedBook::where('reader_id', $reader->id)->where('book_id', $book->id)->exists()) {
                   $book_data[] = [
                       'id'        => $book->id,
                       'name'      => $book->name,
                       'price'     => $book->price ?? 0,
                       'file_name' => $book->thumbnail_image,
           ...
           if (empty($book_data)) {
               return redirect()->route('ebook.frontend', $slug)
                   ->with('error', __('Books already purchased or invalid.'));
           }

           $session_data = $request->session()->get('comapany_stripe_data');
           try {

               $stripe = new \Stripe\StripeClient(!empty(company_setting('stripe_secret', $reader->created_by, $reader->workspace)) ? company_setting('stripe_secret', $reader->created_by, $reader->workspace) : '');
               ...
               $order = new \Workdo\Ebook\Entities\EbookBookOrder();
               $order->order_id          =  '#' . time();
               $order->name              = $reader->first_name ?? null;
               $order->reader_id         = $reader->id;
               $order->book              = json_encode($book_data);
               $order->price             = $total_price;
               $order->payment_type      = 'Stripe';
               $order->payment_status    = 'success';
               $order->receipt           = $receipt_url;
               $order->workspace         = $workspace->id ?? null;
               $order->created_by        = $workspace->created_by ?? null;
               $order->save();

               foreach ($book_data as $book) {
                   $purchased = new \Workdo\Ebook\Entities\EbookPurchasedBook();
                   $purchased->reader_id = $reader->id;
                   $purchased->book_id   = $book['id'];
                   $purchased->order_id  = $order->id;
                   $purchased->save();
               }

               \Workdo\Ebook\Entities\EbookReaderCart::where('reader_id', $reader->id)->delete();

               return redirect()->route('ebook.change.order',  ['slug' => $slug, 'id' =>  \Crypt::encrypt($order->id)])
                   ->with('success',  __('Payment successful.'));
           } catch (\Exception $e) {
               return redirect()->route('ebook.frontend', $slug)->with('error', $e->getMessage());
           }
       }
    ...
    // YogaClasses
    public function YogaCoursesPayWithStripe(Request $request, $slug)
    {
        $products = $request->course_details;
        ...
        $name = isset($member) ? $member->name : (isset($instructor) ? $instructor->name : null);
        ...
        if ($totalprice <= 0) {
            $assignCourse = YogaUtility::DirectAssignCourse($workspace, $products, 'Stripe');
            if ($assignCourse['is_success']) {
                return redirect()->route(
                    'yoga.course.complete',
                    [
                        $slug,
                        \Illuminate\Support\Facades\Crypt::encrypt($assignCourse['courseorder_id']),
                    ]
                )->with('success', __('Transaction has been successfully completed.'));
            } else {
                return redirect()->route('store.cart', $slug)->with('error', __('Something went wrong, Please try again,'));
            }
        }
        ...
        if ($products) {
            try {
                ...
                $return_url_parameters = function ($return_type, $products) {
                    // return '&return_type=' . $return_type;
                    return [
                        'return_type' => $return_type,
                        'course_details' => $products,
                    ];
                };
                /* Initiate Stripe */
                \Stripe\Stripe::setApiKey(company_setting('stripe_secret', $workspace->created_by, $workspace->id));
                ...
                $comapany_stripe_data = \Stripe\Checkout\Session::create(
                    [
                        ...
                        'mode' => 'payment',
                        'metadata' => [
                            'user_id' => isset($name) ? $name : 0,
                            'package_id' => '',
                            'code' => $code,
                        ],
                        'success_url' => route(
                            'get.yoga.courses.pay.status',
                            array_merge(
                                [$slug],
                                $return_url_parameters('success', $products)
                            )
                        ),
                        'cancel_url' => route(
                            'get.yoga.courses.pay.status',
                            array_merge(
                                [$slug],
                                $return_url_parameters('cancel', $products)
                            )
                        ),
                    ]
                );
    ...
    public function getYogaCoursesPaymentStatus($slug, Request $request)
    {
        try {
            if ($request->return_type == 'success') {
                ...
                if ($products) {
                    ...
                    try {
                        $member = Auth::guard('members')->user();
                        $instructor = Auth::guard('instructors')->user();
                        $user = $member ?? $instructor;

                        $course_order = new YogaCourseOrder();
                        $course_order->order_id = '#' . time();
                        $course_order->name = $user->name;
                        $course_order->card_number = '';
                        $course_order->card_exp_month = '';
                        $course_order->card_exp_year = '';
                        $course_order->member_id = $member ? $user->id : null;
                        $course_order->instructor_id = $instructor ? $user->id : null;
                        $course_order->course = json_encode($products);
                        $course_order->price = $totalprice;
                        $course_order->discount_price = !empty($product->discount_price) ? $product->discount_price : '';
                        $course_order->price_currency = !empty(company_setting('defult_currancy', $workspace->created_by, $workspace->id)) ? company_setting('defult_currancy', $workspace->created_by, $workspace->id) : 'USD';
                        $course_order->txn_id = '';
                        $course_order->payment_type = 'Stripe';
                        $course_order->payment_status = 'success';
                        $course_order->receipt = $receipt_url;
                        $course_order->workspace = $workspace->id;
                        $course_order->created_by = $workspace->created_by;
                        $course_order->save();

                        foreach ($products as $course_id) {
                            $product = json_decode($course_id);
                            $purchased_course = new PurchasedYogaCourse();
                            $purchased_course->course_id = $product->id;
                            $purchased_course->member_id = $member ? $member->id : null;
                            $purchased_course->instructor_id = $instructor ? $instructor->id : null;
                            $purchased_course->order_id = $course_order->id;
                            $purchased_course->save();

                            if ($purchased_course->member_id) {
                                $yoga_member = YogaMember::find($purchased_course->member_id);
                                if ($yoga_member) {
                                    $yoga_member->courses_id = $purchased_course->course_id;
                                    $yoga_member->save();
                                }
                            } elseif ($purchased_course->instructor_id) {
                                $yoga_instructor = YogaInstructor::find($purchased_course->instructor_id);
                                if ($yoga_instructor) {
                                    $yoga_instructor->courses_id = $purchased_course->course_id;
                                    $yoga_instructor->save();
                                }
                            }
                        }

                        $type = 'yogacoursepayment';
                        event(new StripePaymentStatus($product, $type, $course_order));
    ...
    public function InvestDepositPayWithStripe(Request $request, $slug)
    {
        $investor_plan = InvestPlan::find($request->plan_id);
        self::payment_setting($investor_plan->created_by, $investor_plan->workspace);
        $data = $request->all();
        $price           = $data['amount'];

        if ($investor_plan->plan_duration_type == 'range') {
            $request_duration = $data['plan_duration'];
            if ($request_duration >= $investor_plan->duration_min && $request_duration <= $investor_plan->duration_max) {
                $plan_duration = $request_duration;
            } else {
                $plan_duration = $investor_plan->duration_min;
            }
        } else {
            $plan_duration = $investor_plan->duration_fix;
        }

        if ($investor_plan->return_type == 'annual_return') {
            $request_return = $data['annual_return'];
            if ($request_return >= $investor_plan->selected_return && $request_return <= $investor_plan->min_return) {
                $annual_return = $request_return;
            } else {
                $annual_return = $investor_plan->min_return;
            }
        } else {
            $annual_return = $investor_plan->fix_return;
        }
        $purchase_date   = $data['purchase_date'];
        $calculation     = InvestorDeposit::calculateExpectedReturn($price,$annual_return,$plan_duration,rtrim($investor_plan->duration_unit, 's'),$purchase_date);
        $expected_return = $calculation['expected_return'];
        $expiry_date     = $calculation['expiry_date'];
        $admin_currancy  = $this->currancy;
        $supported_currencies = ['EUR', 'GBP', 'USD', 'CAD', 'AUD', 'JPY', 'INR', 'CNY', 'SGD', 'HKD', 'BRL'];
        if (!in_array($admin_currancy, $supported_currencies)) {
            return redirect()->back()->with('error', __('Currency is not supported.'));
        ...
        if (isset($this->is_stripe_enabled) && $this->is_stripe_enabled == 'on' && !empty($this->stripe_key) && !empty($this->stripe_secret)) {
            if ($investor_plan) {
                try {
                    ...
                    /* Final price */
                    $stripe_formatted_price = in_array(
                        company_setting('defult_currancy', $investor_plan->created_by, $investor_plan->wokspace),
                        ['MGA', 'BIF', 'CLP', 'PYG', 'DJF', 'RWF', 'GNF', 'UGX', 'JPY', 'VND', 'VUV', 'XAF', 'KMF', 'KRW', 'XOF', 'XPF', 'BRL']
                    ) ? number_format($price, 2, '.', '') : number_format($price, 2, '.', '') * 100;
                    $return_url_parameters = function ($return_type) {
                        return '&return_type=' . $return_type;
                    ...
                    /* Initiate Stripe */
                    \Stripe\Stripe::setApiKey(company_setting('stripe_secret', $investor_plan->created_by, $investor_plan->wokspace));
                    ...
                    $stripe_session = \Stripe\Checkout\Session::create(
                        [
                            ...
                            'line_items' => [
                                [
                                    'price_data' => [
                                        'currency' =>  company_setting('defult_currancy', $investor_plan->created_by, $investor_plan->wokspace),
                                        'unit_amount' => (int)$stripe_formatted_price,
                                        'product_data' => [
                                            'name' => !empty($investor_plan->plan_name) ? $investor_plan->plan_name : 'Basic Package',
                                            'description' => 'stripe payment',
                                        ],
                                    ],
                                    'quantity' => 1,
                    ...
                    session()->put('investor_deposit_session', $data);
                    $request->session()->put('stripe_session', $stripe_session);
                    $stripe_session = $stripe_session ?? false;
                    $stripe_session = $stripe_session ?? false;
                    return new RedirectResponse($stripe_session->url);
                ...
                $msg = __('Investor Not Found.');
                return redirect()->back()->with('error', $msg);
            }
        }
    }
    public function getStripeInvestDepositPaymentStatus(Request $request, $slug)
    {
        $workspace = WorkSpace::where('slug', $slug)->first();
        try {
            $stripe = new \Stripe\StripeClient(!empty(company_setting('stripe_secret', $workspace->created_by, $workspace->id)) ? company_setting('stripe_secret', $workspace->created_by, $workspace->id) : '');
        ...
        try {
            ...
            if ($request->return_type == 'success' && !empty(session()->get('investor_deposit_session'))) {
                $data = session()->get('investor_deposit_session');

                $investor_plan = InvestPlan::find($data['plan_id']);
                $investor = Investor::find($data['investor_id']);


                $existingInvestment = InvestorDeposit::where('investor_id', $investor->id)->where('plan_id', $investor_plan->id)->first();

                if ($existingInvestment) {
                    $existingInvestment->plan_duration = $data['plan_duration'];
                    $existingInvestment->invested_amount = $data['amount'];
                    $existingInvestment->amount = $data['amount'];
                    $existingInvestment->annual_return = $data['annual_return'];
                    $existingInvestment->expected_return = $data['expected_return'];
                    $existingInvestment->payment_type = __('Stripe');
                    $existingInvestment->status = '1';
                    $existingInvestment->receipt = $receipt_url;
                    $existingInvestment->purchase_date = $data['purchase_date'];
                    $existingInvestment->expiry_date = $data['expiry_date'];
                    $existingInvestment->save();
                    $investor_deposit = $existingInvestment;
                } else {
                    $investor_deposit = new InvestorDeposit();
                    $investor_deposit->investor_id = $investor->id;
                    $investor_deposit->plan_id = $investor_plan->id;
                    $investor_deposit->plan_duration = $data['plan_duration'];
                    $investor_deposit->invested_amount = $data['amount'];
                    $investor_deposit->amount = $data['amount'];
                    $investor_deposit->annual_return = $data['annual_return'];
                    $investor_deposit->expected_return = $data['expected_return'];
                    $investor_deposit->payment_type = __('Stripe');
                    $investor_deposit->status = '1';
                    $investor_deposit->receipt = $receipt_url;
                    $investor_deposit->purchase_date = $data['purchase_date'];
                    $investor_deposit->expiry_date = $data['expiry_date'];
                    $investor_deposit->workspace = $workspace->id;
                    $investor_deposit->created_by = $workspace->created_by;
                    $investor_deposit->save();
                }

                //transaction
                $trxID                             = strtoupper(str_replace('.', '', uniqid('', true)));
                $investor_transaction              = new InvestorTransaction();
                $investor_transaction->investor_id = $investor_deposit->investor_id;
                $investor_transaction->plan_id     = $investor_deposit->plan_id;
                $investor_transaction->trx_id      = $trxID;
                $investor_transaction->amount      = $investor_deposit->amount;
                $investor_transaction->type        = 'credit';
                $investor_transaction->detail      = 'Deposit Via - Stripe';
                $investor_transaction->workspace   = $investor_deposit->workspace;
                $investor_transaction->created_by  = $investor_deposit->created_by;
                $investor_transaction->save();

                $type = 'investor_deposit';
                event(new StripePaymentStatus($investor_deposit, $type, $investor_transaction));
                return redirect()->route('investor.transaction', [$slug])->with('success', __('Deposit Pay Successfully.'));
            } else {
                $msg = __('Transaction has been failed');
                return redirect()->back()->with('error', $msg);
            }
